...
function buildICDCodeSystem(concepts) {
  return {
    ...
    id: "icd11-tm2",
    url: "http://id.who.int/icd/entity/146209201", // TM2 root
    version: "2025-09",
    name: "ICD11_TM2",
    status: "active",
...
(async () => {
  try {
    ...
    const selectedEntities = [
      "http://id.who.int/icd/entity/915343154", // Fever of other/unknown origin
      "http://id.who.int/icd/entity/1435254666", // Acute appendicitis
      "http://id.who.int/icd/entity/40398753", // Diabetes mellitus
      "http://id.who.int/icd/entity/761947693", // Hypertension
      "http://id.who.int/icd/entity/1656445230", // Asthma
    ]; // TM2 root
    const concepts = [];
    for (const id of selectedEntities) {
      const entity = await fetchEntity(token, id);
      if (entity) {
        concepts.push(buildICDConcept(entity));
      }
    }
    ...
    const outFile = "icd11/parsed/CodeSystem-icd11-biomedicine.json";

    fs.outputJsonSync(outFile, codeSystem, { spaces: 2 });
    console.log(
      `✅ ICD-11 BioMedicine saved to ${outFile} with ${concepts.length} concepts`
    );
  } catch (err) {
    console.error("❌ Error fetching ICD-11 BioMedicine:", err.message);
  }
})();
